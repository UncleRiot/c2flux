...
public sealed partial class NtfsReader
{
    ...
    public NtfsReader(DriveInfo driveInfo, RetrieveMode retrieveMode)
        : this(driveInfo, retrieveMode, 0)
    {
    }

    /// <summary>
    /// Initializes an <see cref="NtfsReader"/> and synchronously reads the entire MFT
    /// using the requested sequential read-buffer size.
    /// </summary>
    /// <param name="driveInfo">The NTFS drive to read.</param>
    /// <param name="retrieveMode">Flags that control optional metadata loading.</param>
    /// <param name="mftReadBufferSizeBytes">
    /// MFT read-buffer size in bytes. A value of 0 preserves the library default.
    /// </param>
    public NtfsReader(
        DriveInfo driveInfo,
        RetrieveMode retrieveMode,
        uint mftReadBufferSizeBytes)
    {
    ...
    /// <summary>
    /// Returns all named nodes from the already-loaded MFT without resolving full paths
    /// for prefix filtering.
    /// </summary>
    /// <remarks>
    /// Intended for full-volume consumers that already operate on MFT node and parent
    /// indexes and therefore do not need <see cref="GetNodes(string)"/> path filtering.
    /// </remarks>
    public List<INode> GetNodesUnfiltered()
    {
        int nodeCount = _nodes.Length;
        var nodes = new List<INode>(nodeCount);
