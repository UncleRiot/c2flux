...
public sealed partial class NtfsReader : IDisposable
{
    ...
    internal uint _mftReadBufferSizeBytes;
    ...
    {
        // Preserve the library's legacy default unless the caller explicitly
        // requests a larger sequential MFT read buffer.
        uint bufferSize = _mftReadBufferSizeBytes != 0
            ? _mftReadBufferSizeBytes
            : (Environment.OSVersion.Version.Major >= 6 ? 256u : 64u) * 1024;

        uint bytesPerMftRecord = checked((uint)_diskInfo.BytesPerMftRecord);

        if (bufferSize < bytesPerMftRecord)
            bufferSize = bytesPerMftRecord;

        bufferSize -= bufferSize % bytesPerMftRecord;
